...
[[ -n "$(sed 's/#.*//' "$CONF" | tr -d '[:space:]')" ]] || { echo "ERROR: no roles parsed." >&2; exit 1; }

model_for() { roles_conf_get "$CONF" "$1"; }
...
if [[ -n "$avail" ]]; then
  while IFS= read -r line; do
    ...
    m="$(model_for "$role")"; [[ -n "$m" ]] || continue
    [[ "$m" == *:* ]] || m="$m:latest"  # tagless -> implicit :latest
    grep -qxF "$m" <<<"$avail" || echo "WARNING: '$m' is not installed (ollama pull it)." >&2
  done < "$CONF"
fi
...
# tiers.json — consumed by the ollama-delegate MCP server at startup. Unlike
# update_agent's agent-frontmatter path (warns and continues), an empty tier
# is never valid — it would silently write "code": "" and surface later as a
# confusing Ollama 404, so hard-fail here instead.
TIER_CODE="$(model_for tier.code)"
TIER_CHEAP="$(model_for tier.cheap)"
[[ -n "$TIER_CODE" ]]  || { echo "ERROR: roles.conf has no value for 'tier.code' — set it and re-run." >&2; exit 1; }
[[ -n "$TIER_CHEAP" ]] || { echo "ERROR: roles.conf has no value for 'tier.cheap' — set it and re-run." >&2; exit 1; }
{
  echo "{"
  echo "  \"_comment\": \"AUTO-GENERATED by sync-local.sh — edit roles.conf (tier.*) instead.\","
  echo "  \"code\": \"$TIER_CODE\","
  echo "  \"cheap\": \"$TIER_CHEAP\""
  echo "}"
} > "$TIERSFILE"
echo "Wrote $TIERSFILE (code=$TIER_CODE, cheap=$TIER_CHEAP)."

ORCH="$(model_for orchestrator)"; CHEAP="$(model_for cheap)"
# Same rationale as the tier.code/tier.cheap guard above: an empty orchestrator
# would silently write "ANTHROPIC_MODEL=" and surface later as a confusing
# Ollama 404, so hard-fail here instead.
[[ -n "$ORCH" ]] || { echo "ERROR: roles.conf has no value for 'orchestrator' — set it and re-run." >&2; exit 1; }
{
